...
import { WaitlistUpgradeButton } from "./WaitlistUpgradeButton";
...
export default async function WaitlistManagePage({ params }: Props) {
  ...
  const publicUrl = `creatdrop.com/w/${slug}`;
  const publicHref = `https://creatdrop.com/w/${slug}`;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <Link
            href="/waitlist-builder/dashboard"
        ...
        <div className="flex items-center gap-2">
          {!waitlist.isPoweredByHidden && <WaitlistUpgradeButton />}
          <form
            action={async () => {
              "use server";
